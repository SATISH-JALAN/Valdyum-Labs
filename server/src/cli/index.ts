 ...
 * cli/index.ts
 *
 * Valdyum CLI — run agents and manage the 0x402 payment protocol from
 * the terminal.
 *
 * Usage:
 *   valdyum agents list
 *   valdyum agents run <agentId> --input "your prompt"
 *   valdyum agents run <agentId> --input "..." --secret <STELLAR_SECRET>
...
${chalk.cyan('║')}  ${chalk.bold.cyan('        ██║     ╚██████╔╝██║  ██║╚██████╔╝███████╗')}  ${chalk.cyan('║')}
${chalk.cyan('║')}  ${chalk.bold.cyan('        ╚═╝      ╚═════╝ ╚═╝  ╚═╝ ╚═════╝ ╚══════╝')}  ${chalk.cyan('║')}
${chalk.cyan('║')}  ${chalk.bold.cyan('              Valdyum CLI')}                    ${chalk.cyan('║')}
...
const DEFAULT_API_BASE = process.env.VALDYUM_API_URL || 'http://localhost:3000';
const HORIZON_URL =
...
program
  .name('valdyum')
  .description(
    chalk.cyan('Valdyum CLI') +
      ' — Run AI agents with 0x402 Stellar payments from your terminal'
  )
  .version('0.1.0')
  .option('--api <url>', 'Valdyum API base URL', DEFAULT_API_BASE);
...
a2aCmd
  ...
  .action(
    async (
      ...
      try {
        const res = await fetch(`${qstashUrl}/v2/publish/${apiBase}/api/consumers/valdyum-a2a-request`, {
          method: 'POST',
          headers: {
...
program
  ...
  .description('Initialize a new Valdyum agent project')
  .action(async (projectName: string = 'my-agent') => {
    console.log(chalk.bold.cyan(`\n🚀 Initializing Valdyum project: ${projectName}\n`));
...
`
      ...
      agents: [
        {
          ...
          model: 'openai-gpt4o-mini',
          price_xlm: 0.05,
          description: 'Starter Valdyum agent scaffold',
        },
      ],
    });

    writeJsonFile(`${projectName}/config/dashboard.json`, {
      title: 'Valdyum Polymarket Dashboard',
    ...
    writeJsonFile(`${projectName}/.valdyum/dashboard.json`, {
      mode: 'polymarket',
      theme: 'terminal-neon',
    ...
    writeTextFile(
      ...
      `# ${projectName}\n\nValdyum project scaffolded with \`valdyum init\`.\n\n## Quick Start\n\n\`\`\`bash\ncp .env.example .env\nvaldyum agents list\nvaldyum agents run <agentId> --input "your prompt"\nvaldyum dash\n\`\`\`\n`
    );
    ...
    console.log(chalk.gray(`  📄 ${projectName}/.valdyum/dashboard.json`));
    ...
    console.log(chalk.gray('  3. valdyum agents list'));
    console.log(chalk.gray('  4. valdyum dash'));
    console.log(chalk.gray('  5. valdyum agents run <agentId> --input "your prompt" --secret <STELLAR_SECRET>'));
    console.log('');
  });
...
program
  ...
  .action(async (opts: { interval: string }) => {
    ...
    async function render() {
      ...
      if (agents.length === 0) {
        console.log(chalk.bold.white('  │') + chalk.gray('  No agents found. Run valdyum agents list') + chalk.bold.white('                        │'));
      }
      console.log(chalk.bold.white('  └────────────────────────────┴────────────────┴──────────┴────────────┘'));
      console.log('');
